...
void updateControl(){
  ...
  //int myvalue = knob_value >> 2;  // 10 bits (0->1023) shifted right by 2 bits to give 8 bits (0->255)
  int myvalue = map(knob_value, 0, 1023, 1, 200);
  //   aSin.setFreq(myvalue*2); // set the frequency
...
int updateAudio(){
  modDepth = modDepth * myvalue;
    long vibrato = modDepth * oscModulator.next();
    return (int) (oscCarrier.phMod(vibrato)) * amplitude * myvalue;
}
