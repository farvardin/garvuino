/* 
 *  Mozzi + Potentiometers in FM mode  
 * 
...
int knob_0 = 5; // initialisation

const int KNOB_PIN_1 = 1; // set the input for the knob to analog pin 0
int knob_1 = 5; // initialisation
...
void updateControl(){
  ...
  int knob_0_remap = map(knob_0,0,1023, 4000, 12000);

  int knob_1 = analogRead(KNOB_PIN_1); // value is 0-1023
  int knob_1_remap = map(knob_1,0,1023, 100, 800);
  ...
  mod_index = knob_1_remap;
  
  // here's where the smoothing happens
  smoothed_note = kSmoothNote.next(target_note);
  //setFreqs(smoothed_note);
  setFreqs(knob_0_remap);
  //Serial.print("knob_0_remap = ");
  //Serial.println((int)knob_0_remap);

  Serial.print("value = ");
  Serial.println((int)mod_index);
